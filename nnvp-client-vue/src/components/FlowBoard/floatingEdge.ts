...
/**
 * The slice of a Vue Flow GraphNode this module reads (structural, so it stays
 * a pure module — no @vue-flow/core import — and test fakes fit).
 */
...
export interface FloatingEdgeParams {
  ...
  tx: number;
  ty: number;
  sourcePos: PositionValue;
  targetPos: PositionValue;
}
...
function getNodeIntersection(node: FloatingNode, otherNode: FloatingNode): Point {
...
function getEdgePosition(node: FloatingNode, point: Point): PositionValue {
...
export function getEdgeParams(source: FloatingNode, target: FloatingNode): FloatingEdgeParams {
