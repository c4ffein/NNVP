 ...
 * the main thread (./runtime.ts) imports and loops on it.
 *
 * The worker is kept alive across trainings by runtime.ts's singleton: the
...
import type { TraceMeta, TraceResult, WorkerRequest, WorkerResponse } from './protocol';

/** The slice of the Pyodide API this worker touches (dynamic CDN import). */
interface PyodideApi {
  loadPackage(names: string[]): Promise<unknown>;
  // Pyodide interop is genuinely dynamic: results are PyProxy-or-primitive.
...
// project compiles with the DOM lib only, no WebWorker lib alongside).
...
function ensurePyodide(): Promise<PyodideApi> {
  if (!pyodidePromise) {
    pyodidePromise = (async () => {
      ...
      const pyodide: PyodideApi = await loadPyodide({ indexURL: PYODIDE_BASE });
...
async function trace(
  message: Extract<WorkerRequest, { type: 'trace' }>,
): Promise<TraceResult> {
  ...
  const runnerJs: string = result.get('js');
  const evalJs: string = result.get('evalJs');
  const weights: Uint8Array = result.get('weights').toJs(); // Uint8Array
  const meta: TraceMeta = JSON.parse(result.get('meta'));
  result.destroy();
...
self.onmessage = async (event) => {
  ...
  try {
    ...
    throw new Error(`unknown message type "${(message as WorkerRequest).type}"`);
  } catch (error) {
    if (cancelled.delete(message.id)) return;
    const err = error as { message?: unknown } | null | undefined;
    self.postMessage({
      id: message.id, ok: false, error: String(err && err.message ? err.message : err),
    });
  }
};
