...
if (!prefix) throw new Error('usage: bun check_runner.js <prefix> (from run_local.py <prefix>)');
...
async function writeback(device, buf, values) {
...
async function readback(device, buf) {
...
function parseSafetensors(bytes) {
  ...
  const header = JSON.parse(new TextDecoder('utf8').decode(bytes.subarray(8, 8 + headerLength)));
  const tensors = {};
