...
/** A window's position and size, in viewport pixels. */
...
/** The available drag area (usually the viewport's size). */
export interface ViewportSize {
  width: number;
  height: number;
}

/** The dock zones dockZoneAt can propose. */
export type DockZone =
  | 'left' | 'right' | 'bottom' | 'max'
...
export function clampToViewport(rect: WindowRect, viewport: ViewportSize): WindowRect {
  return {
    ...rect,
...
const rememberedRects = new Map<string, WindowRect>();

export function rememberRect(windowId: string | null | undefined, rect: WindowRect): void {
  if (windowId) rememberedRects.set(windowId, { ...rect });
}

export function recallRect(windowId: string | null | undefined): WindowRect | undefined {
  return windowId ? rememberedRects.get(windowId) : undefined;
}

export function resetWindowRects(): void {
...
export interface WindowHandlers {
  getRect(): WindowRect;
  getZone(): DockZone | string | null | undefined;
  applyRect(partialRect: Partial<WindowRect>): void;
}

export interface DockedSibling {
  zone: DockZone | string;
  rect: WindowRect;
  applyRect: WindowHandlers['applyRect'];
}

const liveWindows = new Map<number, WindowHandlers>();
let nextWindowToken = 0;

export function registerWindow(handlers: WindowHandlers): number {
  nextWindowToken += 1;
  liveWindows.set(nextWindowToken, handlers);
  return nextWindowToken;
}

export function unregisterWindow(token: number): void {
  liveWindows.delete(token);
}

export function otherWindowRects(token: number): WindowRect[] {
  const others = [];
...
export function dockedSiblings(token: number): DockedSibling[] {
  const docked = [];
  for (const [key, handlers] of liveWindows) {
...
export function dockZoneAt(
  ...
  viewport: ViewportSize,
  pref: { width: number; height: number } = { width: 300, height: 300 },
  opts: { sideBottomY?: number } = {},
): { zone: DockZone; rect: WindowRect } | null {
  const { x, y } = pointer;
