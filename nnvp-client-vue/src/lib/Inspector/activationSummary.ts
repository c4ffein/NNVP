...
export interface TintPixels {
  kind: 'tint';
  mean: number;
  intensity: number;
}

export type ActivationPixels = GridPixels | TilesPixels | TintPixels;
 ...
 * @param data flat activation values
 * @param shape tensor shape WITHOUT the batch dimension
 * @param opts first conv channel to summarize
...
function normalize(values: ArrayLike<number>, min: number, max: number): Float32Array {
...
function minMax(arrays: ArrayLike<number>[]): [number, number] {
  let min = Infinity;
  let max = -Infinity;
 ...
 */
export function activationToPixels(summary: ActivationSummary): ActivationPixels {
