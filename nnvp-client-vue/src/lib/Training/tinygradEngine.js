...
const unsupported = what => new Error(`${what} is not supported by the tinygrad engine yet`);

// --- Graph probes (exported for the dual-mode tests) -------------------------

function collectLayers(layers, out = []) {
...
const layerName = layer => (layer.kerasLayer ? layer.kerasLayer.name : null);
 ...
 */
export function graphInputShape(graph) {
 ...
 */
export function graphNumClasses(graph) {
  ...
  const byId = new Map(all.map(layer => [String(layer.id), layer]));
...
function createSession({ device, step, meta, graphJson, epochs, makeEvalStep }) {
  let stopRequested = false;
  let evalStepPromise = null;
  return {
...
export function createTinygradEngine({
  ...
  return {
    ...
    async prepare(graphJson, opts) {
      ...
      const step = await instantiate(traced.runnerJs, device, traced.weights);
      ...
      const makeEvalStep = traced.evalJs
        ? () => instantiate(traced.evalJs, device, traced.weights)
        : null;
