...
import type { TrainingSession } from './engine';
import type { KerasLayerJSON, NnvpLayer, NnvpModel } from '../../types/model';

// The tfjs namespace stays untyped here for the same reason as in
...
export function makeSyntheticDataset({ shape, numClasses, samples = 512, valSamples = 128, tf = null }: {
  ...
  const tensorBatch = (images: Float32Array, labels: Int32Array, total: number) => (count: number) => {
 ...
 */
export async function probeMetrics(session: TrainingSession, {
  ...
  batchSize?: number;
  maxBatches?: number;
}): Promise<{ acc: number; loss: number }> {
 ...
 */
export function datasetCompatible(
  boardShape: unknown,
  datasetShape: unknown,
  boardClasses?: number,
  datasetClasses?: number,
): boolean {
...
export function samplesProcessed(engineId: string, epochs: number): number {
...
export function summarizeRun({
  ...
  epochMs?: number[];
  losses: number[];
  finalLogs?: { acc?: number; valLoss?: number; valAcc?: number };
}) {
 ...
 */
export function describeGraph(graphJson: string): { summary: string; finalSoftmax: boolean } {
  const graph = JSON.parse(graphJson) as NnvpModel;
  const layers = (graph.layers || []).filter(
    (l): l is NnvpLayer & { kerasLayer: KerasLayerJSON } => Boolean(l.kerasLayer),
  );
  const parts = layers.map((l) => {
    const { name, parameterValues: p = {} } = l.kerasLayer;
    if (name === 'Input') return `Input[${((p.shape || []) as number[]).join(',')}]`;
    const bits: (string | number)[] = [];
    if (p.units !== undefined) bits.push(p.units as number);
    if (p.filters !== undefined) bits.push(`${p.filters}f`);
    if (p.activation && p.activation !== 'linear') bits.push(p.activation as string);
