/** [first element index, element count, sprite URL] — one sprite chunk. */
export type SpriteEntry = [number, number, string];
...
export default (cdnDir: string): Record<string, DatasetSourceEntry> => {return{
  'MNIST': [
    {
      imagesSpritePath: [...Array(14).keys()].map((x): SpriteEntry =>
  ...
  'CIFAR10': [
    {
      imagesSpritePath: [...Array(30).keys()].map((x): SpriteEntry =>
