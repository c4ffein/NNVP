...
test('traced runners: per-step dropout, live BatchNorm stats, eval weight sync', async ({ page }) => {
  ...
  const result = await page.evaluate(async ({ dropoutModel, bnModel }) => {
    const { getSharedRuntime, acquireWebGpuDevice, instantiateRunner } = await import('/src/lib/TinygradRuntime/runtime.js');
    const { snapshotWeightBufs, writeWeightBuf, syncWeightBufs } = await import('/src/lib/TinygradRuntime/weightIO.js');
    const runtime = getSharedRuntime();
    await runtime.init();
    const device = await acquireWebGpuDevice();

    const trace = async (modelSource, inputShape, numClasses) => {
    ...
    const restore = async (step, snapshot) => {
