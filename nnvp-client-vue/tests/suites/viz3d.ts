...
import type { LayerPlacement, Scene3D } from '../../src/lib/Viz3D/sceneBuild';
import {
...
import type { ActivationSummary, InspectionSnapshot } from '../../src/lib/Viz3D/inspectionBridge';
import type { NnvpLayer, NnvpModel, ParameterValue } from '../../src/types/model';

type DenseSummary = Extract<ActivationSummary, { kind: 'dense' }>;
type ConvSummary = Extract<ActivationSummary, { kind: 'conv' }>;

/** The bridge/scene fakes cover only the fields the code under test reads. */
const asPlacement = (partial: unknown) => partial as LayerPlacement;
const asScene = (partial: unknown) => partial as Scene3D;

const templates = new BoardTemplates().templates;
const templateModel = (name: string): NnvpModel => JSON.parse(templates[name]!);

const expectMat4CloseTo = (
  expect: Expect, actual: ArrayLike<number>, expected: ArrayLike<number>, digits = 6,
) => {
...
logicTest('viz3d math: lookAt from +z is a pure -z translation', ({ expect }) => {
  ...
  const eye: [number, number, number] = [3, -2, 7];
...
logicTest('viz3d scene: sampling covers every source and keeps fan-in plausible', ({ expect }) => {
  ...
  const positionOf = (neuron: number) => scene.neurons.slice(
  ...
  for (let e = 0; e < scene.edgeCount; e += 1) {
    ...
    const sourceIndex = sourcePositions.get(scene.edges.slice(base, base + 3).join(','))!;
    const targetIndex = targetPositions.get(scene.edges.slice(base + 3, base + 6).join(','))!;
    fanOut[sourceIndex] = fanOut[sourceIndex]! + 1;
    fanIn[targetIndex] = fanIn[targetIndex]! + 1;
  }
...
logicTest('viz3d renderer: pipeline descriptors match the scene buffer encoding', ({ expect }) => {
  const module = { label: 'fake-module' } as unknown as GPUShaderModule;
...
logicTest('viz3d bridge: dense summary spreads its block-mean buckets over the grid', ({ expect }) => {
  ...
  const scene = asScene({ layers: [placement], activationSlots: 8 });
  const summary: DenseSummary = {
...
logicTest('viz3d scene: gap tiers — full for transforms, close for same shape, closest for Output', ({ expect }) => {
  const scene = buildScene(templateModel('2D Dense for MNIST'));
  const zOf = (index: number) => scene.layers[index]!.center[2];
  ...
  const zt = (index: number) => tiered.layers[index]!.center[2];
...
logicTest('viz3d bridge: conv slices align their channel window with the summary', ({ expect }) => {
  ...
  const scene = asScene({ layers: [placement], activationSlots: 2 });
  const summary: ConvSummary = {
    kind: 'conv',
    channels: 32,
    channelOffset: 8,
  ...
  const stale: ConvSummary = { ...summary, channelOffset: 0 };
