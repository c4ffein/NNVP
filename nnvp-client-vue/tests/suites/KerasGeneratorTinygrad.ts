...
// --- Fixture builders (mirror KerasGeneratorPyTorch.ts) -----------------
...
function mnistMlpJson(): NnvpModel {
  return {
    inputs: ['1'],
    outputs: ['5'],
    layers: [
...
const method = (name: string, params: Record<string, ParameterValue> = {}): string | null => {
  const graph = { n: { keras_data: { name, parameterValues: params } } } as
    unknown as GeneratorGraph;
