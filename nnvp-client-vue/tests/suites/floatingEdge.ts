/**
 * Floating-edge geometry (pure functions). Migrated from
 * tests/unit/floatingEdge.test.js into the dual registry as logicTest: no
 * app world needed, still executed by BOTH runners.
 */
import { logicTest } from '../harness/define';
import { getEdgeParams } from '../../src/components/FlowBoard/floatingEdge';
import type { FloatingNode } from '../../src/components/FlowBoard/floatingEdge';

// Minimal GraphNode stand-in: floating geometry only reads computedPosition
// (absolute) and dimensions.
const node = (x: number, y: number, width = 100, height = 40): FloatingNode => ({
  computedPosition: { x, y },
  dimensions: { width, height },
});

logicTest('getEdgeParams: anchors on facing vertical borders when nodes are side by side', ({ expect }) => {
...
logicTest('getEdgeParams: anchors on facing horizontal borders when nodes are stacked', ({ expect }) => {
  const source = node(0, 0);
  const target = node(0, 200);
  const {
    sy, ty, sourcePos, targetPos,
  } = getEdgeParams(source, target);
...
logicTest('getEdgeParams: re-anchors when the target moves to the other side', ({ expect }) => {
  const source = node(0, 0);
...
logicTest('getEdgeParams: keeps anchor points on the node borders for diagonal placements', ({ expect }) => {
  const source = node(0, 0);
  ...
  const onBorder = ({ computedPosition: p, dimensions: d }: FloatingNode, x: number, y: number) => (
...
logicTest('getEdgeParams: falls back to centers instead of NaN for unmeasured or concentric nodes', ({ expect }) => {
  ...
  for (const value of [concentric.sx, concentric.sy, concentric.tx, concentric.ty]) {
