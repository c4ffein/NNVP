...
const denseEntry = (name, units, activation, inputShape) => ({
  module: 'keras.layers',
  class_name: 'Dense',
  config: {
    name,
...
const kerasTensor = (shape, sourceName) => ({
...
const makeKerasZip = config => buildZip([
...
const pythonOf = model => new KerasGenerator(JSON.parse(JSON.stringify(model))).generatePythonFromGraph();
