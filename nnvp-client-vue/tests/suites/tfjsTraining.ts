...
// What model.fit() resolves with, as these tests consume it: the History
// object's metric record (loss per epoch).
...
// per epoch.
function expectFiniteLossPerEpoch(expect: Expect, history: FitHistory, epochs: number): void {
...
logicTest('tfjsTraining: trains a functional tf.model({inputs, outputs}) built with .apply() (what generateFunctional() emits)', async ({ expect }) => {
  ...
  const hidden = tf.layers.dense({ units: 8, activation: 'relu' }).apply(input) as SymbolicTensor;
  const output = tf.layers.dense({ units: 1, activation: 'sigmoid' }).apply(hidden) as SymbolicTensor;
