...
function makeFakeDevice() {
  return {
    createBuffer: desc => new FakeBuffer(desc),
...
function makeTraceResult({ batchSize = 32 } = {}) {
  const stateShapes = {
...
function makeFakeRuntime(traceResult = makeTraceResult()) {
  return {
    initCalls: 0,
    traceRequests: [],
...
function makeFakeStep(device, meta, initialValues = {}) {
  const weightBufs = {};
  ...
  const step = async (x, yInt, readLoss = true) => {
...
function leaf(id, name, params = {}, inputLayers = [], outputLayers = []) {
  return {
...
function makeRawDataset({ samples = 320, pixels = 4 } = {}) {
  ...
  return { shape: [pixels], imageByteSize: pixels, trainImages, trainLabels };
}
...
logicTest('tinygradRuntime: one worker, one init, traces answered by id', async ({ expect }) => {
      ...
      if (message.type === 'trace') {
        queueMicrotask(() => this.onmessage({
          data: {
            id: message.id,
            ok: true,
            result: { runnerJs: `runner-for-${message.modelSource}`, weights: new Uint8Array(0), meta: {} },
          },
        }));
      }
    },
  ...
  const posted = worker.posted.find(message => message.type === 'trace');
...
logicTest('tinygradRuntime: worker errors reject the matching trace only', async ({ expect }) => {
  const worker = {
    ...
    postMessage(message) {
      ...
      if (message.type === 'trace') {
        queueMicrotask(() => this.onmessage({
          data: message.modelSource === 'bad'
            ? { id: message.id, ok: false, error: 'Python says no' }
            : { id: message.id, ok: true, result: { runnerJs: 'ok', weights: new Uint8Array(0), meta: {} } },
        }));
      }
    },
  };
...
logicTest('tinygradRuntime: cancel rejects the pending trace and tells the worker', async ({ expect }) => {
  const replies = new Map(); // id -> deliver()
  const worker = {
    posted: [],
    onmessage: null,
    postMessage(message) {
      ...
      if (message.type === 'trace') {
        replies.set(message.id, () => this.onmessage({
          data: { id: message.id, ok: true, result: { runnerJs: 'late', weights: new Uint8Array(0), meta: {} } },
        }));
      }
    },
  };
  ...
  const cancelMessage = worker.posted.find(message => message.type === 'cancel');
...
logicTest('tinygradEngine: prepare tags generation/trace failures as build errors', async ({ expect }) => {
  ...
  const failingRuntime = {
    async init() {},
    trace: () => ({ promise: Promise.reject(new Error('trace boom')), cancel() {} }),
  };
...
logicTest('tinygradEngine: stop() aborts between steps; a throwing callback cancels', async ({ expect }) => {
  ...
  const noRaw = await rejection(session3.fit({ shape: [4] }, {}));
...
logicTest('tinygradEngine: weight I/O enforces the COPY_SRC/COPY_DST usage flags', async ({ expect }) => {
  ...
  const readError = await rejection(readWeightBuf(device, storageOnly));
  expect(readError.message).toContain('COPY_SRC');
  const writeError = await rejection(writeWeightBuf(device, storageOnly, new Float32Array(4)));
  ...
  await writeWeightBuf(device, patched, Float32Array.from([1, 2, 3, 4]));
  expect(Array.from(await readWeightBuf(device, patched))).toEqual([1, 2, 3, 4]);
});
...
logicTest('tinygradEngine: evaluateLogits boots the eval runner lazily and syncs live weights into it', async ({ expect }) => {
  ...
  evalBuf.unmap();
  const evalStep = async (x) => { evalStep.calls.push(x.length); return [new Float32Array([9, 1, 0, 0])]; };
  evalStep.calls = [];
  ...
  const engine = createTinygradEngine({
    ...
    instantiate: async (runnerJs) => {
      ...
      return runnerJs.includes('eval') ? evalStep : step;
    },
  });
  const session = await engine.prepare(JSON.stringify(mnistGraph()), makeOpts());
