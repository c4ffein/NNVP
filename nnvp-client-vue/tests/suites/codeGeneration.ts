 ...
 * (Pure generator coverage runs in both modes via board.js and the
...
async function startErrorTracking(page: Page, canvas: CanvasDriver): Promise<{
  ...
  async ({ page, canvas, expect }) => {
    ...
    for (let i = 0; i < layersOnCanvas.length; i++) {
      ...
      if (rightBarText.includes('Flatten') && rightBarText.includes('data_format')) {
        ...
        const initialValue = await selectInputs[0]!.evaluate(el => (el as HTMLSelectElement).value);
        ...
        await page.waitForTimeout(100);
        const newValue = await selectInputs[0]!.evaluate(el => (el as HTMLSelectElement).value);
