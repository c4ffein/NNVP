...
/** What the mocked POST /api/projects receives (the app's save payload). */
...
e2eOnly(
  ...
  async ({ page, canvas, expect }) => {
    ...
    const state: { projects: MockProject[]; nextId: number; created: CreatedPayload[] } = {
      projects: [
    ...
    await page.route('**/api/**', async (route) => {
      ...
      const post = (): CreatedPayload => (
        req.postDataJSON ? req.postDataJSON() : JSON.parse(req.postData() || '{}')
      );
