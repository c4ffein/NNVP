/**
 * PyTorch code generation. Migrated from tests/unit/KerasGeneratorPyTorch.test.js
...
// --- Fixture builders (mirror KerasGenerator.ts) -----------------------
...
// Intentionally MINIMAL fixtures (only the fields the generator reads), hence
// the `as unknown as` casts up to the persisted types.
...
function unsupportedJson(): NnvpModel {
  return {
    inputs: ['1'],
    outputs: ['3'],
    layers: [
...
function embeddingLstmJson(): NnvpModel {
  return {
    inputs: ['1'],
    outputs: ['4'],
    layers: [
...
function uninferableLstmJson(): NnvpModel {
  return {
    inputs: ['1'],
    outputs: ['4'],
    layers: [
...
// --- PyTorch: full generation -------------------------------------------------

logicTest('kerasGeneratorPyTorch: generates a nn.Module subclass for a sequential chain', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: wires a branching functional graph through forward() dataflow', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: emits a clearly-marked TODO placeholder for an unsupported layer', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: maps a sequential LSTM, unpacking the (output, hidden) tuple in forward()', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: stacks Embedding + recurrent + dense layers in one sequential model', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: infers the LSTM input_size from the upstream Embedding output_dim', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: falls back to units with a loud TODO when input_size cannot be inferred', ({ expect }) => {
...
// --- PyTorch: layer -> torch.nn mapping ----------------------------------------

const ctor = (name: string, params: Record<string, ParameterValue> = {}): string | null => {
  const graph = { n: { keras_data: { name, parameterValues: params } } } as
    unknown as GeneratorGraph;
  return new KerasGeneratorPyTorchHelper(graph, [], [], [], false).moduleConstructor('n');
};

logicTest('kerasGeneratorPyTorch: maps the common layers to lazy modules where shape is unknown', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: maps activations to matching nn modules', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: maps embedding and recurrent layers to their torch modules', ({ expect }) => {
...
logicTest('kerasGeneratorPyTorch: returns null (-> TODO) for an unmapped layer or unknown activation', ({ expect }) => {
