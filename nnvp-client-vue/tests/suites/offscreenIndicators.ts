...
import type { IndicatorNode } from '../../src/lib/FlowInterface/offscreenIndicators';
...
// Dev-only debug handle main.ts installs (same local-typing pattern as the
// canvas driver).
type NnvpDebugWindow = Window & { nnvp: { debug: { graphEditor: FlowGraphEditor } } };

function node(id: string, x: number, y: number, extra: Partial<IndicatorNode> = {}): IndicatorNode {
  return {
    id,
    position: { x, y },
