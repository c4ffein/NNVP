 ...
 * behavior; code generation lives in codeGeneration.dual.js, datasets and
 * training in training.dual.js).
 *
 ...
 */
import { appTest, e2eOnly } from '../define';
...
e2eOnly(
  ...
  'The graph substance (adding a layer) is appTest-covered in board.dual.js; this wrap preserves the original assertion that the browser console/pageerror streams stay clean during the real catalog click — only observable in a live browser.',
...
e2eOnly(
  ...
  'Multi-add substance is appTest-covered in board.dual.js; this wrap preserves the clean browser console/pageerror assertion across the sequential real catalog clicks, which needs a live browser.',
  async ({ page, canvas, expect }) => {
    const { consoleErrors } = await startErrorTracking(page, canvas);
    // Click Dense layer
    const denseLayer = await page.$('.LayerTemplate:has-text("Dense")');
...
e2eOnly(
  ...
  'Undo substance is appTest-covered in board.dual.js; this wrap drives undo through the real Edit dropdown menu chrome and preserves the clean browser-console assertion.',
  async ({ page, canvas, expect }) => {
    const { consoleErrors } = await startErrorTracking(page, canvas);
    // Add two layers
    const denseLayer = await page.$('.LayerTemplate:has-text("Dense")');
...
e2eOnly(
  ...
  'Redo substance is appTest-covered in board.dual.js; this wrap drives undo/redo through the real Edit dropdown menu chrome and preserves the clean browser-console assertion.',
  async ({ page, canvas, expect }) => {
    const { consoleErrors } = await startErrorTracking(page, canvas);
    // Add a layer
    const denseLayer = await page.$('.LayerTemplate:has-text("Dense")');
