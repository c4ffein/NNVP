/**
 * Board behavior: one definition, both modes (see tests/dual/define.js).
...
appTest('moving a layer persists in the model', async ({ board, expect }) => {
  await board.addLayer('Dense');
  const before = JSON.parse(await board.graphJSON()).layers[0];
  await board.moveLayer(before.id, 321, 123);
  const after = JSON.parse(await board.graphJSON()).layers[0];
...
appTest('the board generates TensorFlow code in both languages', async ({ board, expect }) => {
  ...
  const python = new KerasGenerator(JSON.parse(json)).generatePythonFromGraph();
  const javascript = new KerasGenerator(JSON.parse(json)).generateJavascriptFromGraph();
