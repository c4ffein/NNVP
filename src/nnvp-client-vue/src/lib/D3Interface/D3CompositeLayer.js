 ...
 */
export default function D3CompositeLayer(id, x, y, childs) {
  let thisComposite = this;
  let htmlID = "Composite_" + id;
  let name = "Block_" + id;
  D3ComponentLayer.call(thisComposite, id, x, y, name, htmlID);
  thisComposite.class = "D3CompositeLayer";
  thisComposite.d3node = null;
  thisComposite.isOpen = false;
  childs.forEach(child => {
    thisComposite.childs.push(child);
    child.addObserver(thisComposite);
    child.parent = thisComposite;
  });
  thisComposite.graph = null;
  thisComposite.deleteState = false;
};

D3CompositeLayer.prototype = Object.create(D3ComponentLayer.prototype);

D3CompositeLayer.prototype.clone = function () {
  let res = new D3CompositeLayer(this.id, this.x, this.y, this.name, this.htmlID);
 ...
 * @returns a JSON data
 */
D3CompositeLayer.prototype.toJSON = function () {
 ...
 */
D3CompositeLayer.prototype.update = function (observable) {
  let thisComposite = this;
 ...
 */
D3CompositeLayer.prototype.remove = function () {
 ...
 */
D3CompositeLayer.prototype.delete = function (graph) {
  let thisComposite = this;
 ...
 */
D3CompositeLayer.prototype.removeInputLayer = function (layer) {
  let thisComposite = this;
  D3ComponentLayer.prototype.removeInputLayer.call(thisComposite, layer);
 ...
 * @param layer which Layer to remove
 */
D3CompositeLayer.prototype.removeOutputLayer = function (layer) {
  let thisComposite = this;
  D3ComponentLayer.prototype.removeOutputLayer.call(thisComposite, layer);
  thisComposite.childs.forEach(child => child.removeOutputLayer(layer));
};

D3CompositeLayer.prototype.drawLayer = function (d3node, graph) {
  let thisComposite = this;
  ...
  thisComposite.childs.forEach(child => {
    ...
    if (child.class === "D3CompositeLayer") {
      child.close();
    }
 ...
 */
D3CompositeLayer.prototype.drawComposite = function (d3node, graph) {
...
D3CompositeLayer.prototype.setDrag = function (graph, gElement) {
 ...
 */
D3CompositeLayer.prototype.dragged = function (eventX, eventY) {
 ...
 */
D3CompositeLayer.prototype.open = function (graph) {
  let thisComposite = this;
  let x = thisComposite.x;
  let y = thisComposite.y;
 ...
 */
D3CompositeLayer.prototype.close = function (graph) {
  let thisComposite = this;
  thisComposite.width = 180;
  thisComposite.height = 60;
 ...
 */
D3CompositeLayer.prototype.break = function (graph) {
  let thisComposite = this;
...
D3CompositeLayer.loadJSON = function(json, graph) {
  var newChilds = [];
  json.childs.forEach(child => newChilds.push(graph.loadComposite(child, graph)));
  let newLayer = new D3CompositeLayer(json.id, json.x, json.y, newChilds);
  newChilds.forEach(child => { child.parent = newLayer; });
  ...
  newLayer.d3node = graph.svgD3CompositeLayers;
  return newLayer;
};

D3CompositeLayer.prototype.getAllContainedLayers = function () {
  let res = [];
 ...
 */
D3CompositeLayer.prototype.getAllContainedJSON = function () {
  let res = [];
  this.childs.forEach(child => {
    if (child.class === "D3Layer") {
